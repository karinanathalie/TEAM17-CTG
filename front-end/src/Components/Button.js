...
import LocationComponent from "./Distance";
...
export const BackButton = ({ onClick }) => {
  return (
    <button
      className="font-poppins flex space-x-[10px] w-[135px] h-[48px] px-[21px] py-[10px] rounded-[8px] bg-buttonblack hover:drop-shadow-lg active:drop-shadow-none"
      onClick={onClick}
    >
...
export const VolunteerParticipantToggle = ({ onClick }) => {
  ...
  return (
    <label className="font-poppins relative flex justify-center items-center w-[289px] h-[50px] rounded-[8px] cursor-pointer peer">
      ...
      <span className="flex flex-col justify-center z-10 px-[40px] relative text-pastelyellow font-semibold dark:text-gray-300 peer-checked:text-black">
        Volunteer
      </span>
      <span className="flex flex-col justify-center z-10 px-[40px] relative text-black font-semibold dark:text-gray-300 peer-checked:text-blue">
        Participant
      </span>
    </label>
  );
...
export const ButtonAccount = ({ onClick, text, location }) => {
  return (
