            ...
            <ContentWrapper className="w-full flex flex-col px-[53px]">
                ...
                <EventListWrapper className="w-full flex flex-col">
                    {list_of_events.map((event) => {
                        return(
                            <EventCard date={event.event_date} location={event.event_location} eventPicture={event.event_image} eventName={event.event_name} 
                            eventDescription = {event.event_description}
                            eventSummary={event.event_summary} role={current_role}/>
                        )
                    })}
                </EventListWrapper>
            </ContentWrapper>
